...
int on_decrypt(
    ...
    if (unencrypted_data_key->buffer == NULL) {
        ...
        if (props != NULL) {
            assert(aws_byte_buf_is_bounded(unencrypted_data_key, props->data_key_len));
...
int main() {
    ...
    ensure_trace_has_allocated_records(&keyring_trace, 1);
    assert(
    ...
    assert(aws_cryptosdk_edk_list_is_bounded(&edks, NUM_ELEMS));
    assert(aws_cryptosdk_edk_list_is_valid(&edks));
    assert(aws_cryptosdk_edk_list_elements_are_bounded(&edks, SIZE_MAX));
    assert(aws_cryptosdk_edk_list_elements_are_valid(&edks));

    struct aws_hash_table *enc_ctx = can_fail_malloc(sizeof(*enc_ctx));
    if (enc_ctx != NULL) {
        ensure_allocated_hash_table(enc_ctx, MAX_TABLE_SIZE);
        assert(aws_hash_table_is_valid(enc_ctx));
        ensure_hash_table_has_valid_destroy_functions(enc_ctx);
        size_t empty_slot_idx;
        assert(aws_hash_table_has_an_empty_slot(enc_ctx, &empty_slot_idx));
    }
