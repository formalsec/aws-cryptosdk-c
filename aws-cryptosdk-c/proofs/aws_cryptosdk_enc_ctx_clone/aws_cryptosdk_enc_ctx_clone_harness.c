...
void aws_cryptosdk_enc_ctx_clone_harness() {
    ...
    assert(aws_hash_table_is_valid(dest));
    assert(dest->p_impl->entry_count <= MAX_TABLE_SIZE);
    ensure_hash_table_has_valid_destroy_functions(dest);

    ensure_allocated_hash_table(src, MAX_TABLE_SIZE);
    assert(aws_hash_table_is_valid(src));
    assert(src->p_impl->entry_count <= MAX_TABLE_SIZE);
