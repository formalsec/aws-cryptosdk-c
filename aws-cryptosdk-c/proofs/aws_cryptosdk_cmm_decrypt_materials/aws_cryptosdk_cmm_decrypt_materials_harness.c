...
void aws_cryptosdk_cmm_decrypt_materials_harness() {
    ...
    assert(aws_cryptosdk_edk_list_is_bounded(&request->encrypted_data_keys, MAX_NUM_ITEMS));
    assert(aws_cryptosdk_edk_list_is_valid(&request->encrypted_data_keys));

    // Stub until https://github.com/diffblue/cbmc/issues/5344 is fixed
    // edk_list Precondition: The list has valid list elements
    /*
      assert(aws_cryptosdk_edk_list_elements_are_bounded(&request->encrypted_data_keys, MAX_STRING_LEN));
      ensure_cryptosdk_edk_list_has_allocated_list_elements(&request->encrypted_data_keys);
      assert(aws_cryptosdk_edk_list_elements_are_valid(&request->encrypted_data_keys));
    */
    assert(aws_cryptosdk_dec_request_is_valid(request));
